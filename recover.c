...
long GetFsector(char *argv[]){
	...
	skipNum = 1009*be->BPB_NumFATs*be->BPB_BytsPerSec;
...
void listDir(char *argv[]){
	...
	fseek(fp, skipNum, 0);
	//right now the pointer's position is at the root directory
	fread(buf, sizeof(buf), 1, fp);
	while(buf[0]!=0){ //buf[0] will not be 0 except there is nothing in the field
		if(buf[0]==65){ //judge whether the file name is validly short name buf[0]is Ox41
			...
			if((int)(de->DIR_FileSize)>0){ //if the checked entry is file's, print out the info
				startClustor = de->DIR_FstClusHI+de->DIR_FstClusLO;
				i++;
				...
				while((de->DIR_Name[1])!=0){
					if(buf[0]==65){
						fread(buf, sizeof(buf), 1, fp);
						de = (struct DirEntry *)buf;
						if((int)de->DIR_FileSize>0){
							startClustor = de->DIR_FstClusHI+de->DIR_FstClusLO;
							i++;
							trim(buf,(int)de->DIR_FileSize);
...
struct DirEntry* findFile(char *argv[]){
	...
	fread(buf, sizeof(buf), 1, fp);
	// printf("------buf[0]:%d----\n",buf[0]);
	for(rowNum=1;buf[0]!=0;rowNum++){ //search to the end of root directory
